...
app.post('/login', (req, resp) => {
    ...
    } else if(body.username != 'usuario' || body.password != 'teste123') {
        resp.status(401).send({ auth: false, message: 'Usuário ou senha inválida' });
    } else {
        resp.status(403).send({ auth: false, message: 'Sem permissão' });
    }
})
